...
if ($PullData) {
    ...
    if ($pendingUpdatesData.Total -gt 0) {
        ...
        $message += "Estimated MAX time to install updates: $estimatedTimeFormatted`n"
    } else {
...
$showPrompt = $false
if ($SkipUptimeCheck -or $uptime.TotalDays -gt 2 -or $config.ExecutionCount -ge $MaxExecutions) {
    $showPrompt = $true
}
...
if ($pendingUpdatesData.Total -gt 0) {
    ...
    $message += "`nEstimated MAX time to install updates: $estimatedTimeFormatted`n"
} else {
